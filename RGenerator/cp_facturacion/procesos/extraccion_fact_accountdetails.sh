#!/bin/bash
#########################################################################################################
# NOMBRE: extraccion_fact_accountdetails.sh		     	      								            #
# DESCRIPCION:																							#
#  Shell que realiza el proceso de importacion la tabla accountdetails de Oracle a la tabla				#
#  otc_t_accountdetails en Hive																			#
...
VAL_DIA=`date '+%Y%m%d'` 
VAL_HORA=`date '+%H%M%S'` 
VAL_LOG=$VAL_RUTA/logs/OTC_T_ACCOUNTDETAILS_$VAL_DIA$VAL_HORA.log
...
echo "==== Inicia ejecucion del proceso OTC_T_ACCOUNTDETAILS ===="`date '+%Y%m%d%H%M%S'` >> $VAL_LOG
...
--vfilesql=$VAL_RUTA/sql/otc_t_accountdetails.sql 2>&1 &>> $VAL_LOG
...
echo "==== Finaliza ejecucion del proceso OTC_T_ACCOUNTDETAILS ===="`date '+%Y%m%d%H%M%S'` >> $VAL_LOG
