#!/bin/bash
#########################################################################################################
# NOMBRE: extraccion_fact_revenuecode.sh	 	     	      								            #
# DESCRIPCION:																							#
#  Shell que realiza el proceso de importacion la tabla revenuecode de Oracle a la tabla 				#
#  otc_t_contact en Hive																				#
# AUTOR: Karina Castro - Softconsulting                            										#
# FECHA CREACION: 2022-12-05   																			#
# PARAMETROS DEL SHELL                            													    #
# VAL_FECHAEJE				Fecha ejecucion en formato YYYYMM											#						
#########################################################################################################
# MODIFICACIONES																						#
# FECHA  		AUTOR     		DESCRIPCION MOTIVO														#
#########################################################################################################
ENTIDAD=OTC_T_REVENUECODE
...
VAL_DIA=`date '+%Y%m%d'` 
VAL_HORA=`date '+%H%M%S'` 
VAL_LOG=$VAL_RUTA/logs/OTC_T_REVENUECODE_$VAL_DIA$VAL_HORA.log
...
--vfilesql=$VAL_RUTA/sql/otc_t_revenuecode.sql \
--vcampoparte=$VAL_CAMPO_PT \
--vvalorparte=$VAL_FECHAEJE >> $VAL_LOG

#VALIDA EJECUCION DEL ARCHIVO SPARK
error_spark=`egrep 'IndexError:|An error occurred|Caused by:|pyspark.sql.utils.ParseException|AnalysisException:|NameError:|IndentationError:|Permission denied:|ValueError:|ERROR:|error:|unrecognized arguments:|No such file or directory|Failed to connect|Could not open client' $VAL_LOG | wc -l`
...
echo "==== Finaliza ejecucion del proceso OTC_T_REVENUECODE ===="`date '+%Y%m%d%H%M%S'` >> $VAL_LOG
