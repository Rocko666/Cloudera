#!/bin/bash
#########################################################################################################
# NOMBRE: extraccion_fact_accountattribute.sh 	     	      								            #
# DESCRIPCION:																							#
#  Shell que realiza el proceso de importacion la tabla accountattribute de Oracle a la tabla			#
#  otc_t_accountattribute en Hive																		#
...
#########################################################################################################
ENTIDAD=OTC_T_ACCOUNTATTRIBUTE
...
VAL_DIA=`date '+%Y%m%d'` 
VAL_HORA=`date '+%H%M%S'` 
VAL_LOG=$VAL_RUTA/logs/OTC_T_ACCOUNTATTRIBUTE_$VAL_DIA$VAL_HORA.log

#VALIDACION DE PARAMETROS INICIALES
if [ -z "$ENTIDAD" ] || [ -z "$VAL_KINIT" ] || [ -z "$VAL_RUTA_SPARK" ] || [ -z "$VAL_RUTA_LIB" ] || [ -z "$VAL_RUTA_IMP_SPARK" ] || [ -z "$VAL_NOM_IMP_SPARK" ] || [ -z "$VAL_NOM_JAR_ORC_11" ] || [ -z "$TDDB_RBM" ] || [ -z "$TDHOST_RBM" ] || [ -z "$TDPASS_RBM" ] || [ -z "$TDPORT_RBM" ] || [ -z "$TDSERVICE_RBM" ] || [ -z "$TDUSER_RBM" ] || [ -z "$VAL_RUTA" ]  || [ -z "$HIVEDB" ] || [ -z "$HIVETABLE" ] || [ -z "$VAL_TIPO_CARGA" ] || [ -z "$VAL_MASTER" ] || [ -z "$VAL_DRIVER_MEMORY" ] || [ -z "$VAL_EXECUTOR_MEMORY" ] || [ -z "$VAL_NUM_EXECUTORS" ] || [ -z "$VAL_JDBCURL" ] || [ -z "$VAL_LOG" ]; then
	echo " ERROR: - uno de los parametros esta vacio o nulo"
	exit 1
fi

echo "==== Inicia ejecucion del proceso OTC_T_ACCOUNTATTRIBUTE ===="`date '+%Y%m%d%H%M%S'` >> $VAL_LOG
...
--vfilesql=$VAL_RUTA/sql/otc_t_accountattributes.sql 2>&1 &>> $VAL_LOG
...
echo "==== Finaliza ejecucion del proceso OTC_T_ACCOUNTATTRIBUTE ===="`date '+%Y%m%d%H%M%S'` >> $VAL_LOG
